/*
 * Have an issue with the scaffold-dbcontext, if we run the command to generate all models from database, the ColiksContext will be override by the generation.
 * An solution is to make a second ColiksContext (just a copy paste of the generated one) and name it ColiksContext2. After, user this file (ColiksContext2) for the whole application.
 * So you can add personal code here without loosing it with a new scaffold of the database.
 * Related issues : 
